...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            shiny::checkboxInput(inputId = "dose", label = "Original dose response?", value = F),
            
            # serial dilution dose response?
            shiny::checkboxInput(inputId = "serialdose", label = "Free serial dose response?", value = F),
            
            # serial dilution dose response (with dilution factor)
            shiny::checkboxInput(inputId = "serialdose_df", label = "Dilution factor dose response?", value = F),
...
server <- function(input, output, session) {
    ...
    setup_plates <- reactive({
        ...
        output$setup <- shiny::renderUI({
            ...
            if(input$dose == T | input$serialdose == T) {
                ...
                lapply(1:numdrugs, function(i) {
                    inputPanel(
                        tagList(
                            ...
                            # drug concentration
                            shiny::textInput(inputId = glue::glue("drug_dose{i}"), label = "Highest drug concentration (uM):"),
                            
                            # number of drug concentrations
                            shiny::numericInput(inputId = glue::glue("numconcs{i}"), label = "Number of drug concentrations (including control):", min = 2, value = 5),
                            
                            # dilution factor
                            shiny::numericInput(inputId = glue::glue("dilution_factor{i}"), label = "Dilution factor (e.g. 2 = two-fold dilutions):", min = 0, value = 2),
    ...
    shiny::observeEvent(input$drug_done, {
        ...
        if(input$dose == T | input$serialdose == T | input$serialdose_df == T) {
    ...
    calculate_dose <- reactive({
        ...
        if(input$dose == T) {
            ...
            # } 
                ...
                while(nrow(tooSmall) > 0) {
                    ...
                    concs2 <- dose2 %>%
                        dplyr::filter(drug %in% unique(tooSmall$drug)) %>%
                        dplyr::mutate(intconc = ifelse(wellconc == max(wellconc), intconc / 10, intconc))
                ...
                for(i in 2:nrow(dose)) {
                    ...
                    if(dose$wellconc[i] == max(dose$wellconc)) {
                        new_volume <- (dose$control_dil_ul[i] + dose$drug_ul[j])*100*1.1
                    } else {
                        new_volume <- dose$control_dil_ul[i] + dose$drug_ul[j]*1.1
                    }
            ...
            # CHANGE THIS
            drugplate <- dose %>%
                dplyr::mutate(wellconc = round(wellconc, digits = 4),
                              intconc = round(intconc, digits = 7),
                              intconc = ifelse(intconc*1000 == drugstock, "Stock concentration", intconc * 1000),
            ...
            for(i in 1:numdrugs) {
                df <- data.frame(
                    ...
                    drugstock = input[[glue::glue("drug_stock{i}")]],
                    diluent = input[[glue::glue("control_name{i}")]],
                    highconc = input[[glue::glue("drug_dose{i}")]],
                    numconcs = input[[glue::glue("numconcs{i}")]],
                    DF = input[[glue::glue("dilution_factor{i}")]],
                    version = input$version,
                    drugplates = input[[glue::glue("drugplates{i}")]],
                    controlplates = 0,
                    wells = input[[glue::glue("wells{i}")]],
        ...
        if(input$food == "Lysate") {
            ...
            if(input$version == "v2") {
                ...
                lysate_K <- (paste0("Add ",  round(lysate / 5000 * 5, digits = 2) - (lysate / 5000), " mL of K medium to HB101"))

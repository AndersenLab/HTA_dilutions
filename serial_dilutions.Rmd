...
Excess_Percent = 10
DilutionFactors = c(2,1.5,2,2,2,2)
DrugList = c("Methyl mercury","Aldicarb","Methomyl","Chlorfenapyr","Parathion","Pyraclostrobin")
Drugs = length(DrugList)
#DilutionFactors = rep_len(Dilution_Factor, Drugs)

High_Dose_List_uM = c(250,
                      1000,
                      400,
                      24,
                      400,
...
Drug_Stock_Concentration_mM = c(150,
                                2000,
                                1000,
                                500,
                                250,
...
n_microcentrifuge <- ((OD100totalvolume*0.001/7.5) - round(OD100totalvolume*0.001/7.5, digits = 0))/0.8
```
...
V_foodperdose = 25*Strains*Reps*(1+(Excess_Percent/100))    # Well mixture volume (worms, OD30, drug, diluent)
...
###########
V_Stock = ((C_drugdilution)*(V_drugdilution + V_transferred))/(stock[i])
V_totaldiluent <- (V_drugdilution + V_transferred)-V_Stock
High_Dose_Table <- data.frame(rbind(V_Stock, V_totaldiluent))
options(scipen = 99999)
rownames(High_Dose_Table) <- c(paste("Volume of", stock[i]/1000, "mM", drug[i], "Stock",sep = " "),
                               "Volume of Diluent")
